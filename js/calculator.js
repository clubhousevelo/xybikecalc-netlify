class BikeCalculator {
    ...
    initializeEventListeners() {
        ...
        addBikeBtn.addEventListener('click', async () => await this.addBike());
        ...
        firebase.auth().onAuthStateChanged(async user => {
            if (user) {
                ...
                await this.refreshBikeCardsAfterLogin();
    ...
    handleDragStart(bikeCard, startEvent) {
        ...
        const handleEnd = () => {
            ...
            // Save the new bike order to localStorage
            this.saveData();
    ...
    async addBike() {
        ...
        await this.updateCalculationsForBike(bikeData.id);
    ...
    async addManualBike() {
        const bikeData = {
            ...
            _userModifiedSpacersHeight: false,
            _userModifiedHeadsetHeight: false,
            // Manual bikes don't have stock values - set to undefined to prevent highlighting
            _stockStemHeight: undefined,
            _stockStemLength: undefined,
            _stockStemAngle: undefined,
            _stockSpacersHeight: undefined,
            _stockHeadsetHeight: undefined
        };
        ...
        return `
            ...
            <div class="geometry-section">
                <div class="input-group">
                    <label>Reach:</label>
                    <input type="number" class="reach ${readonlyClass}" value="" ${!isManual ? 'readonly' : ''} min="300" max="600">
                    <span>mm</span>
                </div>
                <div class="input-group">
                    <label>Stack:</label>
                    <input type="number" class="stack ${readonlyClass}" value="" ${!isManual ? 'readonly' : ''} min="400" max="800">
                    <span>mm</span>
                </div>
                <div class="input-group">
                    <label>Head Tube Angle:</label>
                    <input type="number" class="hta ${readonlyClass}" value="" ${!isManual ? 'readonly' : ''} step="0.1" min="60" max="80">
                    <span>°</span>
                </div>
                <div class="input-group">
                    <label>Seat Tube Angle:</label>
                    <input type="number" class="sta ${readonlyClass}" value="" ${!isManual ? 'readonly' : ''} step="0.1" min="60" max="85">
                    <span>°</span>
                </div>
                <div class="input-group">
                    <label>Seat Tube Length:</label>
                    <input type="number" class="stl ${readonlyClass}" value="" ${!isManual ? 'readonly' : ''} min="250">
                    <span>mm</span>
                </div>
            </div>
            <div class="stem-section">
                ...
                <div class="input-group">
                    ...
                    <input type="number" class="headset-height" value="10" min="0">
                    <span>mm</span>
                </div>
            </div>
    ...
    initializeBikeCardInputs(bikeCard, bikeData, index) {
        ...
        if (bikeData.isManual) {
            ...
            card.querySelector('.stem-height').value = bikeData.stemHeight;
    ...
    async updateBikeData(bikeId) {
        ...
        bike.stemHeight = stemHeightValue === '' ? 0 : parseFloat(stemHeightValue);
    ...
    async updateCalculations() {
        // If backend calc flag is enabled, pre-compute handlebar positions server-side
        if (window && window.USE_BACKEND_CALC && typeof APIClient !== 'undefined') {
            try {
                const targetHandlebarX = parseFloat(document.getElementById('targetHandlebarX').value) || 0;
                const targetHandlebarY = parseFloat(document.getElementById('targetHandlebarY').value) || 0;
                const targetSaddleX = Math.abs(parseFloat(document.getElementById('targetSaddleX').value)) || 0;
                const targetSaddleY = parseFloat(document.getElementById('targetSaddleY').value) || 0;

                // Prepare a light copy of bikes used for calculation
                const bikesForServer = this.bikes
                    .filter(b => b && !b.isDisabled)
                    .map(b => ({
                        id: b.id,
                        brand: b.brand || '',
                        model: b.model || '',
                        size: b.size || '',
                        reach: parseFloat(b.reach) || 0,
                        stack: parseFloat(b.stack) || 0,
                        hta: parseFloat(b.hta) || 0,
                        sta: parseFloat(b.sta) || 0,
                        stl: parseFloat(b.stl) || 0,
                        stemLength: b.stemLength !== undefined && b.stemLength !== '' ? parseFloat(b.stemLength) : 0,
                        stemAngle: b.stemAngle !== undefined && b.stemAngle !== '' ? parseFloat(b.stemAngle) : 0,
                        spacersHeight: b.spacersHeight !== undefined && b.spacersHeight !== '' ? parseFloat(b.spacersHeight) : 0,
                        stemHeight: b.stemHeight !== undefined && b.stemHeight !== '' ? parseFloat(b.stemHeight) : 0,
                        headsetHeight: b.headsetHeight !== undefined && b.headsetHeight !== '' ? parseFloat(b.headsetHeight) : 0,
                        handlebarReach: parseFloat(b.handlebarReach) || 0,
                        saddleSetback: parseFloat(b.saddleSetback) || 0,
                        saddleHeight: parseFloat(b.saddleHeight) || 0
                    }));

                if (bikesForServer.length > 0) {
                    const serverResult = await APIClient.callCalculator('xy-position', {
                        bikes: bikesForServer,
                        targetReach: targetHandlebarX,
                        targetStack: targetHandlebarY,
                        targetSaddleX: targetSaddleX,
                        targetSaddleY: targetSaddleY
                    });

                    if (serverResult && Array.isArray(serverResult.results)) {
                        // Attach ALL server-computed values to matching bikes
                        serverResult.results.forEach(r => {
                            const match = this.bikes.find(b => b.id === r.id);
                            if (match) {
                                match._serverHandlebarX = r.handlebarX;
                                match._serverHandlebarY = r.handlebarY;
                                match._serverReachDiff = r.reachDiff;
                                match._serverStackDiff = r.stackDiff;
                                match._serverTotalDiff = r.totalDiff;
                                match._serverSetbackVsSTA = r.setbackVsSTA;
                                match._serverEffectiveSTA = r.effectiveSTA;
                                match._serverBbToSRC = r.bbToSRC;
                                match._serverBbToRail = r.bbToRail;
                                match._serverExposedSeatpost = r.exposedSeatpost;
                                match._serverVisualCoords = r.visualCoords;
                            }
                        });
                    }
                }
            } catch (e) {
                // If server fails, log error
                console.error('Server calculation failed:', e);
            }
        }

        // Update calculations for all bikes (will use server values if present)
        this.bikes.forEach(async bike => {
            await this.updateCalculationsForBike(bike.id);
        });
    }
    ...
    async updateCalculationsForBike(bikeId) {
        ...
        // Check if we have target saddle X/Y for basic saddle calculations (Effective STA and BB to SRC)
        const hasBasicSaddleRequirements = targetSaddleX && targetSaddleY;
        
        // Check if we have target saddle X/Y and STA for full saddle calculations
        const hasFullSaddleRequirements = targetSaddleX && targetSaddleY && bike.sta;
        ...
        if (hasRequiredGeometry || hasBasicSaddleRequirements) {
            // If backend calc is enabled, trigger server calculation for this specific bike
            if (window && window.USE_BACKEND_CALC && typeof APIClient !== 'undefined') {
                try {
                    let result;
                    
                    if (hasRequiredGeometry) {
                        // Full calculation with handlebar and saddle calculations
                        const requestData = {
                            bikes: [{
                                id: bike.id,
                                brand: bike.brand || '',
                                ...
                                hta: parseFloat(bike.hta) || 0,
                                sta: parseFloat(bike.sta) || 0,
                                stl: parseFloat(bike.stl) || 0,
                                stemLength: bike.stemLength !== undefined && bike.stemLength !== '' ? parseFloat(bike.stemLength) : 0,
                                stemAngle: bike.stemAngle !== undefined && bike.stemAngle !== '' ? parseFloat(bike.stemAngle) : 0,
                                spacersHeight: bike.spacersHeight !== undefined && bike.spacersHeight !== '' ? parseFloat(bike.spacersHeight) : 0,
                                stemHeight: bike.stemHeight !== undefined && bike.stemHeight !== '' ? parseFloat(bike.stemHeight) : 0,
                                headsetHeight: bike.headsetHeight !== undefined && bike.headsetHeight !== '' ? parseFloat(bike.headsetHeight) : 0,
                                handlebarReach: parseFloat(bike.handlebarReach) || 80,
                                saddleSetback: parseFloat(bike.saddleSetback) || 0,
                                saddleHeight: parseFloat(bike.saddleHeight) || 0
                            }],
                            targetReach: targetHandlebarX,
                            targetStack: targetHandlebarY,
                            targetSaddleX: targetSaddleX,
                        ...
                        result = await APIClient.callCalculator('xy-position', requestData);
                    } else if (hasBasicSaddleRequirements) {
                        // Saddle-only calculation
                        result = await APIClient.callCalculator('seatpost', {
                            saddleX: targetSaddleX,
                            saddleY: targetSaddleY,
                            seatTubeAngle: parseFloat(bike.sta) || 0,
                            seatTubeLength: parseFloat(bike.stl) || 0
                        });
                    }
                    
                    if (result) {
                        if (hasRequiredGeometry && result.results && result.results.length > 0) {
                            // Full calculation results
                            const bikeResult = result.results[0];
                            bike._serverHandlebarX = bikeResult.handlebarX;
                            bike._serverHandlebarY = bikeResult.handlebarY;
                            bike._serverSetbackVsSTA = bikeResult.setbackVsSTA;
                            bike._serverEffectiveSTA = bikeResult.effectiveSTA;
                            bike._serverBbToSRC = bikeResult.bbToSRC;
                            bike._serverBbToRail = bikeResult.bbToRail;
                            bike._serverExposedSeatpost = bikeResult.exposedSeatpost;
                        } else if (hasBasicSaddleRequirements) {
                            // Saddle-only calculation results
                            bike._serverSetbackVsSTA = result.setbackVsSTA;
                            bike._serverEffectiveSTA = result.effectiveSTA;
                            bike._serverBbToSRC = result.bbToSRC;
                            bike._serverBbToRail = result.bbToRail;
                            bike._serverExposedSeatpost = result.exposedSeatpost;
                        }
                    }
                } catch (error) {
                    console.error('Server calculation error for bike:', bike.id, error);
                }
                
            }
            
            // Use server-computed values (backend calculations only)
            if (window && window.USE_BACKEND_CALC && bike._serverHandlebarX != null && bike._serverHandlebarY != null) {
                // Convert NaN and null values to "--"
                handlebarX = (isNaN(bike._serverHandlebarX) || bike._serverHandlebarX === 'NaN' || bike._serverHandlebarX === null) ? '-- ' : bike._serverHandlebarX;
                handlebarY = (isNaN(bike._serverHandlebarY) || bike._serverHandlebarY === 'NaN' || bike._serverHandlebarY === null) ? '-- ' : bike._serverHandlebarY;
            } else {
                // Server calculation required
                handlebarX = '-- ';
                handlebarY = '-- ';
                console.warn('Server calculation not available for bike:', bike.id);
            }
        ...
        // Use server-computed values (backend calculations only)
        if (window && window.USE_BACKEND_CALC && bike._serverSetbackVsSTA != null) {
            // Convert NaN and null values to "--"
            setbackVsSTA = (isNaN(bike._serverSetbackVsSTA) || bike._serverSetbackVsSTA === 'NaN' || bike._serverSetbackVsSTA === null) ? '--' : bike._serverSetbackVsSTA;
            effectiveSTA = (isNaN(bike._serverEffectiveSTA) || bike._serverEffectiveSTA === 'NaN' || bike._serverEffectiveSTA === null) ? '--' : bike._serverEffectiveSTA;
            bbToSRC = (isNaN(bike._serverBbToSRC) || bike._serverBbToSRC === 'NaN' || bike._serverBbToSRC === null) ? '-- mm' : bike._serverBbToSRC;
            bbToRail = (isNaN(bike._serverBbToRail) || bike._serverBbToRail === 'NaN' || bike._serverBbToRail === null) ? '-- mm' : bike._serverBbToRail;
            exposedSeatpost = (isNaN(bike._serverExposedSeatpost) || bike._serverExposedSeatpost === 'NaN' || bike._serverExposedSeatpost === null) ? '-- mm' : bike._serverExposedSeatpost;
        }

        // Update display
        if (hasRequiredGeometry) {
            // Handle handlebar X display
            if (handlebarX === '-- ' || isNaN(handlebarX) || handlebarX === null) {
            ...
            // Handle handlebar Y display
            if (handlebarY === '-- ' || isNaN(handlebarY) || handlebarY === null) {
        ...
        // Saddle calculations can be displayed if we have basic saddle requirements OR required geometry
        if (hasBasicSaddleRequirements || hasRequiredGeometry) {
            // Setback vs STA - only if we have full saddle requirements (needs STA)
            card.querySelector('.setback-sta').textContent = 
                (hasFullSaddleRequirements && typeof setbackVsSTA === 'number') ? 
                `${setbackVsSTA > 0 ? '+' : ''}${setbackVsSTA} mm` : '--';
            
            // Effective STA - can calculate with just Target Saddle XY
            card.querySelector('.effective-sta').textContent = 
                effectiveSTA !== '--' ? `${effectiveSTA}°` : '--';
            
            // BB to SRC - can calculate with just Target Saddle XY
            card.querySelector('.bb-rail').textContent = 
                bbToSRC !== '--' ? `${bbToSRC} mm` : '-- mm';
            
            // BB to Rail - only if we have full saddle requirements (needs STA)
            card.querySelector('.bb-to-rail').textContent = 
                (hasFullSaddleRequirements && bbToRail !== '--') ? `~${bbToRail} mm` : '-- mm';
            
            // Exposed Seatpost - only if we have full saddle requirements (needs STA and ST Length)
            card.querySelector('.exposed-seatpost').textContent = 
                (hasFullSaddleRequirements && bike.stl && exposedSeatpost !== '--') ? 
                `${exposedSeatpost} mm` : '-- mm';
        } else {
            // If we don't have saddle requirements, display placeholder values for saddle calculations
            card.querySelector('.setback-sta').textContent = '--';
            card.querySelector('.effective-sta').textContent = '--';
            card.querySelector('.bb-rail').textContent = '--';
            card.querySelector('.bb-to-rail').textContent = '--';
    ...
    async resetBike(bikeId) {
        ...
        bike.headsetHeight = 10; // Default headset height
        ...
        card.querySelector('.bb-to-rail').textContent = '-- mm';
    ...
    deleteBike(bikeId) {
        ...
        // Clear the bike's selection state from localStorage
        try {
            const stemSelections = JSON.parse(localStorage.getItem('xyStemSelections') || '{}');
            delete stemSelections[`stem_${bikeId}`];
            localStorage.setItem('xyStemSelections', JSON.stringify(stemSelections));
        } catch (e) {
            console.warn('Could not clear bike selection from localStorage:', e);
        }
    ...
    async duplicateBike(bikeId) {
        ...
        // For manual bikes, clear stock values to prevent highlighting
        if (duplicatedBike.isManual) {
            duplicatedBike._stockStemLength = undefined;
            duplicatedBike._stockStemAngle = undefined;
            duplicatedBike._stockSpacersHeight = undefined;
            duplicatedBike._stockStemHeight = undefined;
            duplicatedBike._stockHeadsetHeight = undefined;
        }
        ...
        if (card) {
            ...
            // Apply highlighting for stock values if they exist (not null) - only for non-manual bikes with actual geometry data
            if (!duplicatedBike.isManual && duplicatedBike.brand && duplicatedBike.model && duplicatedBike.size) {
        ...
        await this.updateCalculationsForBike(duplicatedBike.id);
        this.saveData();
        this.updateXYStemComparisonGraph();
    }
    ...
    async setupBikeSelectors(bikeId) {
            ...
            if (sizeSelect.value && firebase.auth().currentUser) {
                ...
                if (geometry) {
                    if (bike) {
                        ...
                        if (!isInitialSetup) {
                            // Apply stock values from the database, or use defaults if no stock value provided
                            ...
                            if (geometry.headsetHeight !== null) {
                                ...
                                bike.headsetHeight = 0; // Default value
        ...
        // Add event listeners to track manual changes to stem configuration
        // These flags are used to track user modifications for future reference
        ...
        if (stemHeightInput) {
            stemHeightInput.addEventListener('input', async () => {
                if (bike) {
                    bike.stemHeight = parseFloat(stemHeightInput.value) || 0;
                    bike._userModifiedStemHeight = true; // Mark as user-modified
                    
                    // Check if value differs from stock value and remove highlighting if so (only for non-manual bikes)
                    if (!bike.isManual && bike._stockStemHeight !== undefined && bike.stemHeight !== bike._stockStemHeight) {
        ...
        if (stemLengthInput) {
            stemLengthInput.addEventListener('input', async () => {
                if (bike) {
                    bike.stemLength = parseFloat(stemLengthInput.value) || 0;
                    bike._userModifiedStemLength = true; // Mark as user-modified
                    
                    // Check if value differs from stock value and remove highlighting if so (only for non-manual bikes)
                    if (!bike.isManual && bike._stockStemLength !== undefined && bike.stemLength !== bike._stockStemLength) {
        ...
        if (stemAngleInput) {
            stemAngleInput.addEventListener('input', async () => {
                if (bike) {
                    bike.stemAngle = parseFloat(stemAngleInput.value) || 0;
                    bike._userModifiedStemAngle = true; // Mark as user-modified
                    
                    // Check if value differs from stock value and remove highlighting if so (only for non-manual bikes)
                    if (!bike.isManual && bike._stockStemAngle !== undefined && bike.stemAngle !== bike._stockStemAngle) {
        ...
        if (spacerHeightInput) {
            spacerHeightInput.addEventListener('input', async () => {
                if (bike) {
                    bike.spacersHeight = parseFloat(spacerHeightInput.value) || 0;
                    bike._userModifiedSpacersHeight = true; // Mark as user-modified
                    
                    // Check if value differs from stock value and remove highlighting if so (only for non-manual bikes)
                    if (!bike.isManual && bike._stockSpacersHeight !== undefined && bike.spacersHeight !== bike._stockSpacersHeight) {
        ...
        if (headsetHeightInput) {
            headsetHeightInput.addEventListener('input', async () => {
                if (bike) {
                    bike.headsetHeight = parseFloat(headsetHeightInput.value) || 0;
                    bike._userModifiedHeadsetHeight = true; // Mark as user-modified
                    
                    // Check if value differs from stock value and remove highlighting if so (only for non-manual bikes)
                    if (!bike.isManual && bike._stockHeadsetHeight !== undefined && bike.headsetHeight !== bike._stockHeadsetHeight) {
        ...
        const data = {
            ...
            bikes: this.bikes.map(bike => {
                ...
                return {
                    ...
                    sta: bike.sta || '',
                    stl: bike.stl || '',
                    stemLength: bike.stemLength !== undefined ? bike.stemLength : 100,
                    stemAngle: bike.stemAngle !== undefined ? bike.stemAngle : -6,
                    spacersHeight: bike.spacersHeight !== undefined ? bike.spacersHeight : 20,
                    stemHeight: bike.stemHeight !== undefined ? bike.stemHeight : 40,
                    headsetHeight: bike.headsetHeight !== undefined ? bike.headsetHeight : 0,
                    ...
                    if (bikesToLoad.length === 0) {
                        ...
                        bikesToLoad.forEach((bikeData, index) => {
                            ...
                            if (card) {
                                ...
                                card.querySelector('.headset-height').value = bikeData.headsetHeight !== undefined && bikeData.headsetHeight !== '' ? bikeData.headsetHeight : 0;
                                
                                // Apply highlighting for stock values if they exist (not null) - only for non-manual bikes
                                if (!bikeData.isManual) {
            ...
            bikes: validBikes.map(bike => {
                ...
                return {
                    ...
                    calculatedValues: {
                        ...
                        bbToSRC: card.querySelector('.bb-rail').textContent.split(' ')[0],
                        bbToRail: card.querySelector('.bb-to-rail').textContent.split(' ')[0],
    ...
    async refreshBikeCardsAfterLogin() {
    ...
    drawXYStemVisualization(stemData, allStems) {
        ...
        stemData.forEach(stem => {
            ...
            let x0 = reach;
            let y0 = stack;
            // Spacer end (starts after headset height)
            let x1 = x0 + Math.cos(htaRad) * (stem.headsetHeight + stem.spacerHeight);
            let y1 = y0 + Math.sin(htaRad) * (stem.headsetHeight + stem.spacerHeight);
            // Stem center
        ...
        stemData.forEach((stem) => {
            ...
            if (stem.spacerHeight > 0) {
                ...
                ctx.lineWidth = 5;
                ctx.beginPath();
                // Spacer starts after headset height
                const spacerStartX = x0 + Math.cos(htaRad) * stem.headsetHeight;
                const spacerStartY = y0 + Math.sin(htaRad) * stem.headsetHeight;
                ctx.moveTo(tx(spacerStartX), ty(spacerStartY));
                ...
                if (numDashes > 0) {
                    ...
                    for (let i = 1; i <= numDashes; i++) {
                        ...
                        if (dashDistance < stem.spacerHeight) {
                            const dashX = spacerStartX + Math.cos(htaRad) * dashDistance;
                            const dashY = spacerStartY + Math.sin(htaRad) * dashDistance;
            ...
            // Draw headset height segment as trapezoid (extends head tube upward)
            if (stem.headsetHeight > 0) {
                ctx.fillStyle = '#bbb'; // Same color as head tube
                ctx.strokeStyle = '#999'; // Slightly darker border
                ctx.lineWidth = 1;
                
                // Calculate trapezoid dimensions
                const bottomWidth = 8; // mm - wider bottom edge
                const topWidth = 6;     // mm - narrower top edge
                const headsetEndX = x0 + Math.cos(htaRad) * stem.headsetHeight;
                const headsetEndY = y0 + Math.sin(htaRad) * stem.headsetHeight;
                
                // Calculate perpendicular angles for width
                const perpAngle = htaRad + Math.PI / 2;
                
                // Bottom edge points (wider)
                const bottomLeftX = x0 + Math.cos(perpAngle) * (bottomWidth / 2);
                const bottomLeftY = y0 + Math.sin(perpAngle) * (bottomWidth / 2);
                const bottomRightX = x0 - Math.cos(perpAngle) * (bottomWidth / 2);
                const bottomRightY = y0 - Math.sin(perpAngle) * (bottomWidth / 2);
                
                // Top edge points (narrower)
                const topLeftX = headsetEndX + Math.cos(perpAngle) * (topWidth / 2);
                const topLeftY = headsetEndY + Math.sin(perpAngle) * (topWidth / 2);
                const topRightX = headsetEndX - Math.cos(perpAngle) * (topWidth / 2);
                const topRightY = headsetEndY - Math.sin(perpAngle) * (topWidth / 2);
                
                // Draw trapezoid
                ctx.beginPath();
                ctx.moveTo(tx(bottomLeftX), ty(bottomLeftY));
                ctx.lineTo(tx(bottomRightX), ty(bottomRightY));
                ctx.lineTo(tx(topRightX), ty(topRightY));
                ctx.lineTo(tx(topLeftX), ty(topLeftY));
                ctx.closePath();
                ctx.fill();
                ctx.stroke();
            }
    ...
    updateXYStemComparisonGraph() {
        ...
        .map((bike, idx) => {
            ...
            let headsetHeight = bike.headsetHeight;
            if (headsetHeight === '' || headsetHeight === null || isNaN(headsetHeight)) {
                headsetHeight = 0;
            } else {
                headsetHeight = parseFloat(headsetHeight);
            }
            const reach = parseFloat(bike.reach) || 0;
            const stack = parseFloat(bike.stack) || 0;
            
            // Use server-calculated visualization coordinates (backend only)
            let x0, y0, x1, y1, x2, y2, x3, y3;
            if (window && window.USE_BACKEND_CALC && bike._serverVisualCoords) {
                x0 = bike._serverVisualCoords.x0;
                y0 = bike._serverVisualCoords.y0;
                x1 = bike._serverVisualCoords.x1;
                y1 = bike._serverVisualCoords.y1;
                x2 = bike._serverVisualCoords.x2;
                y2 = bike._serverVisualCoords.y2;
                x3 = bike._serverVisualCoords.x3;
                y3 = bike._serverVisualCoords.y3;
            } else {
                // Backend calculation required - use defaults
                x0 = reach;
                y0 = stack;
                x1 = reach;
                y1 = stack;
            ...
            return { id: bike.id, headTubeAngle, stemHeight, stemLength, stemAngle, spacerHeight, headsetHeight, reach, stack, label, x3, y3, colorIndex: idx };
        });
